...
draft: true
---
After faltering due to a combination of increasingly difficult puzzles,
holiday fun, and the start of a new freelance gig, I'm hoping to get
back to my un-done Advent 2021 days (four of them, after this one).

I had actually solved [Day 22](https://adventofcode.com/2021/day/22) by Christmas
but never got around to optimizing or blogging it. It was a struggle to get the
runtime down to something measurable in minutes; I hoped (in vain) to do some cleanup
and find some optimizations before posting.
```clojure
(ns aoc.2021.day.22
...
The problem involves a three-dimensional space and cuboids parallel
to its three axes. The cuboids are various sizes and shapes, and they
overlap. Our 3D space begins with all 1x1x1 "cells" being "off"; we then
process a series of instructions to turn the cells in various cuboid regions
on or off. The question: at the end, how many cells are on?
```clojure
...
For part 1, I modeled space as a set of individual cells, which was very
practical for our 100x100x100-cell region.
```clojure
...
(rcf/tests
  (time (part-1 instr)) := 612714)
```

But of course, as heavily hinted to in part 1, part 2 asks us to consider a much
larger region -- the large example input yields an outcome of 2,758,514,936,282,235
"on" cells -- for which the dumb approach is feasible in terms of neither time nor space.

It's been a couple of weeks now since I wrote this code, and it is *gnarly* stuff -- I'll try
to make sense of it as I go and clean up where it's easy, but no promises.

To make this more fun, as I revisit this code, I'm going to try adding some property-based tests
to cover some of the cases for which I'd used post-conditions during initial writing.
```clojure
(defn qc-pass [& results]
  (if-let [failure (first (filter (complement :pass?)
                                  results))]
    failure
    true))
...
(def seg-gen (gen/fmap sort (gen/vector gen/small-integer 2)))
(rcf/tests
  ...
  (qc-pass
    (tc/quick-check 100
        (prop/for-all [a seg-gen
                       b seg-gen]
          (= (segments-disjoint? a b)
             (segments-disjoint? b a)))))
  := true)
```

This last property test ensures that `segments-disjoint?` returns the same result
regardless of the order in which its two arguments are given.

Now to determine whether two cuboids are disjoint:
```clojure
...
(def cuboid-gen (gen/vector seg-gen 3))
(rcf/tests
  ...
  (qc-pass
    (tc/quick-check 100
       (prop/for-all [a cuboid-gen
                      b cuboid-gen]
         (= (cuboids-disjoint? a b)
            (cuboids-disjoint? b a))))
    (tc/quick-check 100
       (prop/for-all [c cuboid-gen]
         (false? (cuboids-disjoint? c c)))))
  := true
  ,)
```

We lift-and-shift the property test from `segments-disjoint?`, and also add one to
assert that a cuboid is never disjoint with itself.

Getting into stranger, probably inelegant realms: a function that takes two overlapping
line segments returns a sequence of segments split on each boundary:
```clojure
(defn overlapping-seg-subs [a b]
  ...
  (let [[a b] (sort (map vec [a b]))
    ...
    (cond
      (= a b) [a]
      (= a1 b1) [[a1 a2] [(inc a2) b2]]
      ;; Know that ax1 < bx1
      (= a2 b2) [[a1 (dec b1)] [b1 b2]]
      (= a2 b1) [[a1 (dec a2)] [a2 a2] [(inc b1) b2]]
      (> a2 b2) [[a1 (dec b1)] [b1 b2] [(inc b2) a2]]
      (> a2 b1) [[a1 (dec b1)] [b1 a2] [(inc a2) b2]]
      :else (throw (ex-info "Segments are disjoint!" {:segs [a b]})))))

(def overlapping-segs-gen (gen/such-that (fn [[a b]] (not (segments-disjoint? a b)))
                                         (gen/vector seg-gen 2)))
(rcf/tests
  (overlapping-seg-subs [0 1] [0 1]) := [[0 1]]
  (overlapping-seg-subs [0 1] [0 2]) := [[0 1] [2 2]]
  (overlapping-seg-subs [0 2] [1 2]) := [[0 0] [1 2]]
  (overlapping-seg-subs [0 3] [1 2]) := [[0 0] [1 2] [3 3]]
  (overlapping-seg-subs [0 2] [1 3]) := [[0 0] [1 2] [3 3]]
  (overlapping-seg-subs [0 2] [2 3]) := [[0 1] [2 2] [3 3]]
  (overlapping-seg-subs [0 2] [2 2]) := [[0 1] [2 2]]
  (qc-pass
    (tc/quick-check 100
                    (prop/for-all [[a b] overlapping-segs-gen]
                      (= (overlapping-seg-subs a b)
                         (overlapping-seg-subs b a)))))
  := true
  ,)
```

Some more self-explanatory helpers:
```clojure
  ...
  (qc-pass
    (tc/quick-check 100
       (prop/for-all [c cuboid-gen]
         (cuboid-contains? c c)))
    (tc/quick-check 100
       (prop/for-all [[a b] (gen/vector-distinct cuboid-gen {:min-elements 2
                                                             :max-elements 2})]
         (not (and (cuboid-contains? a b)
                   (cuboid-contains? b a))))))
...
Now, a function that takes two cuboids and splits them up on
intersection boundaries, returning a sequence of cuboids that covers
the same set of cells with no overlaps.
```clojure
(defn sub-cuboids [a b]
  ...
  (if (cuboids-disjoint? a b)
    ...
    (let [[ax ay az] a
          [bx by bz] b
          xs (overlapping-seg-subs ax bx)
          ys (overlapping-seg-subs ay by)
          zs (overlapping-seg-subs az bz)
...
Now a function that takes a seq of `cuboids` and removes
all of those contained within the cuboid `remove-me`:
```clojure
(defn remove-cuboid [cuboids remove-me]
  {:post (>= (volume remove-me)
             (- (reduce + (map volume cuboids))
                (reduce + (map volume %))))}
  (filter #(not (cuboid-contains? remove-me %))
          cuboids))
```

`slow-union` takes a collection of cuboids and returns their
"union" such that all cells are covered, with all overlapping
cuboids in the input set being split up on boundaries.
```clojure
(defn slow-union [[c & cuboids :as all-cuboids]]
  {:post [(and (<= (reduce + (map volume %))
                   (reduce + (map volume all-cuboids)))
               (->> (combo/combinations % 2)
                    (not-any? (fn [[a b]] (or (cuboid-contains? a b)
                                              (cuboid-contains? b a))))))]}
  (loop [acc #{c}
         [c & more-cuboids] cuboids]
    (if (nil? c)
      acc
      (let [same-cuboids? (fn [as bs] (= (set as) (set bs)))
            [e novel-cuboids] (some (fn [e]
                                      (let [sub-cuboids (sub-cuboids e c)]
                                        (when (not (same-cuboids? [e c] sub-cuboids))
                                          [e sub-cuboids])))
                                    acc)]
        (cond
          novel-cuboids (recur (disj acc e)
                               (into more-cuboids novel-cuboids))
          :fully-disjoint (recur (conj acc c) more-cuboids))))))
```

Fast union does similarly, guaranteeing that each cell is "covered"
only once in the output, but it doesn't necessarily split everything
up on boundaries.

This is useful when we're turning a region "on" -- if the region is
already on, just leave it as-is instead of slicing and dicing up the
cuboids on their boundaries.
```clojure
...
Function that attempts to merge two cuboids, in the
hopes that they're adjacent and same-dimensioned on
one axis:
```clojure
...
(defn merge-into [cuboids-set cuboid]
  (let [[old merged] (some #(let [merged (merge-cuboids % cuboid)]
                              (when (not= [% cuboid] merged)
                                [% merged]))
                           cuboids-set)]
    (if merged
      (into (disj cuboids-set old) merged)
      (conj cuboids-set cuboid))))
```

The last piece, where we execute the instructions.
```clojure
    ...
    (reduce (fn [acc [op & cuboid]]
              (let [f (if (= op "off")
                    ...
                    cs (if (= op "off")
                         ...
                         cs)
                    cs (reduce (fn [acc c] (merge-into acc c))
                               #{}
                               (set cs))]
...
(rcf/tests
  (part-2 (filter #(within-bound? % 50) t-instr-larger)) := 474140
  (time (part-2 t-instr-larger)) := 2758514936282235  ; => 111148.810791 ms
  (time (part-2 instr)) := 1311612259117092
  ,)
```

The code is gross... but we're not building a church. I think my high-level
approach is sound and the inefficiencies lie in the implementation, mainly in
the `union` functions -- but I'm excited to see others' solutions.
